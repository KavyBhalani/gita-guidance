...
for (const file of files) {
  ...
  // Check if navbar has blog
  const navbarSectionMatch = content.match(/navbar:\s*\{([^}]+)\}/);
  if (navbarSectionMatch && !navbarSectionMatch[1].includes('blog:')) {
    const word = trans[code] || "Blog";
    const newContent = content.replace(/(navbar:\s*\{[^}]*?about:\s*"[^"]+",)/, `$1\n    blog: "${word}",`);
